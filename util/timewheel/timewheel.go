...
type Timer struct {
	status     *concurrent.Int64 // Timer status.
...
type wheel struct {
	...
	slots      []*concurrent.List // Slot array.
	number     int64              // Slot Number=len(slots).
	ticks      *concurrent.Int64  // Ticked count of the wheel, one tick is one of its interval passed.
...
func New(slot int, interval time.Duration, level ...int) *Timer {
	...
	t := &Timer{
		status:     concurrent.NewInt64(StatusRuning),
...
func (t *Timer) AddEntry(interval time.Duration, job JobFunc, singleton bool, times int, status int) *Entry {
	return t.doAddEntry(interval, job, singleton, int64(times), int64(status))
}
...
func (t *Timer) AddTimes(interval time.Duration, times int, job JobFunc) *Entry {
	return t.doAddEntry(interval, job, true, int64(times), StatusReady)
}
...
func (t *Timer) Start() {
	t.status.Set(StatusRuning)
}

// Stop stops the timer.
func (t *Timer) Stop() {
	t.status.Set(StatusStoped)
}
...
func (t *Timer) doAddEntry(interval time.Duration, job JobFunc, singleton bool, times int64, status int64) *Entry {
